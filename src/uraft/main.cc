...
void parseOptions(int argc, char **argv, uRaftController::Options &opt, bool &make_daemon, std::string &pidfile) {
	...
	po::store(po::command_line_parser(argc, argv).
	          options(cmdline_options).positional(positional_options).run(), variable_map);
	...
	if (variable_map.count("config") != 0) {
		...
		if (!ifs) {
			safs::log_err("Can not open configuration file: {}", strerror(errno));
			exit(EXIT_FAILURE);
		}

		po::store(po::parse_config_file(ifs, config_file_options, true), variable_map);
		po::notify(variable_map);
	}
...
int getSeed() {
	...
	if (readAmount != sizeof(result)) {
		result = (int)std::time(nullptr);
	}
...
bool daemonize() {
	...
	if (dup(1) < 0) {
		safs::log_critical("Unable to duplicate stdout descriptor: {}", strerror(errno));
		return false;
	}
...
int main(int argc, char **argv) {
	...
	if (const char *logLevelChar = std::getenv("SAUNAFS_LOG_LEVEL")) {
		...
		if (result) {
			...
			safs::log_critical("Invalid SAUNAFS_LOG_LEVEL variable: {}", result.error());
			return EXIT_FAILURE;
		}
	} else {
		// This is needed for the helper script
		setenv("SAUNAFS_LOG_LEVEL", safs::log_level_to_string(logLevel).data(), 1);
	}
	safs::add_log_syslog();
