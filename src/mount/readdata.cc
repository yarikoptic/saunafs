...
std::unique_ptr<IMemoryInfo> memoryInfo = createMemoryInfo();

bool readShouldWaitForSystemMemory(size_t bytesToReadLeft) {
	std::unique_lock lock(gUsedReadCacheMemoryMutex);
	uint64_t avalaibleSystemMemory = memoryInfo->getAvailableMemory();
...
void waitForEnoughSystemMemory(size_t bytesToReadLeft) {
	while (readShouldWaitForSystemMemory(bytesToReadLeft)) {
		fprintf(stderr,
			"Not enough system memory available for reading. Waiting for "
			"enough memory...\n");
		usleep(1000);
	}
}

uint32_t getBytesToBeReadFromCS(uint32_t index, uint32_t offset,
									uint32_t size, const uint64_t fileLength) {
...
void read_data_init(uint32_t retries,
	...
	gReadCacheMaxSize.store(std::min<uint64_t>(
	    read_chache_max_size_mB * (1024LL * 1024LL), memoryInfo->getTotalMemory()));
...
int read_to_buffer(ReadRecord *rrec, uint64_t current_offset,
	...
	while (bytes_to_read > 0) {
		...
		try {
			...
			waitForEnoughSystemMemory(read_cache_bytes_to_reserve);
